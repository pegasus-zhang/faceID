...
public:
    FaceMatcher();
    ~FaceMatcher();
    FaceMatcher(const cv::cuda::GpuMat& face_feature, const std::vector<std::string>& face_id);
    int Init(const std::string& dataset_path);
    ...
    int ParseDatasetFile(const std::string& dataset_path,cv::cuda::GpuMat& face_feature, std::vector<std::string>& face_name_list) const;
private:
