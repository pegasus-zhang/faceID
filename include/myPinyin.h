...
*/
int getComPingyinForStr(const std::wstring& inStr, std::wstring& outFrist_py, std::wstring& outFull_py);//ok
/*
...
*/
std::wstring getNamePingyin(const std::wstring& inStr, bool isLastName);//ok
/*
0. 自动切分 姓、名
...
3. eg:
	wcout << L"姓名拆分测试：" << endl;
	std::wstring names[13] = { L"东皇太一", L"尉迟小白", L"解波", L"卜艾", L"颜碧玉", L"句帅", L"杨红给", L"吕布", L"亚里士多缺德", L"覃黄埔", L"菊花拉姆", L"上官万", L"mack" };
	for (size_t i = 0; i < 13; i++)
	{
		std::wstring full, fist, last;
		full = names[i];
	...
	mack         => 姓:     ,名:mack
*/
void myNameSplit(const std::wstring& inFullName, std::wstring& outLastName, std::wstring& outFirstName);//ok
...
*/
int getAllPingyinForWord(const std::wstring& inWord, std::vector<std::wstring>& outFrist_py, std::vector<std::wstring>& outFull_py); 
/*
...
*/
std::wstring getNoRepeatPingyinForWord(const std::wstring& inWord);
/* 
0. 检查是否是汉字
...
std::string GetChineseName(const std::string& str);