...
namespace YoloGpu
{
    ...
    static __global__ void GpuMatToContinues(cv::cuda::PtrStepSz<uchar3> in, uint8_t* out)
    {
   ...
   // 用于存储元数据的全局变量  
    ...
    static std::vector<uint8_t> load_file(const std::string& file) {  
        ...
        // 读取元数据长度（前 4 个字节）  
        int meta_len = 0;  
        in.read(reinterpret_cast<char*>(&meta_len), sizeof(int));  
        if (in.gcount() != sizeof(int)) {  
            std::cerr << "Failed to read metadata length from file: " << file << std::endl;  
            return {};  
        }  

        // 读取元数据内容  
        if (meta_len > 0) {  
            std::vector<char> meta_buffer(meta_len);  
            in.read(meta_buffer.data(), meta_len);  
            if (in.gcount() != meta_len) {  
                std::cerr << "Failed to read metadata from file: " << file << std::endl;  
                return {};  
            }  
            g_metadata = std::string(meta_buffer.begin(), meta_buffer.end());  
        } else {  
            g_metadata.clear(); // 如果没有元数据，清空全局元数据  
            std::cerr << "No metadata found in file: " << file << std::endl;  
        }  

        // 打印元数据内容  
        if (!g_metadata.empty()) {  
            std::cout << "Metadata: " << g_metadata << std::endl;  
        } else {  
        ...
        size_t remaining_length = in.tellg() - (sizeof(int) + meta_len);  
        if (remaining_length <= 0) {  
            std::cerr << "No remaining data found in file: " << file << std::endl;  
            return {};  
        }  

        std::vector<uint8_t> data(remaining_length);  
        in.seekg(sizeof(int) + meta_len, std::ios::beg); // 移动指针到元数据之后  
        in.read(reinterpret_cast<char*>(data.data()), remaining_length);  
        if (in.gcount() != remaining_length) {  
            std::cerr << "Failed to read remaining data from file: " << file << std::endl;  
            return {};  
        }  
