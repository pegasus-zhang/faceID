...
namespace MatrixRobotVision
{
    ...
    class Infer{
    public:
        virtual std::shared_future<IScrfd::BoxArray> commit(const cv::Mat& image) = 0;
        virtual std::vector<std::shared_future<IScrfd::BoxArray>> commits(const std::vector<cv::Mat>& images) = 0;
    };
    ...
    std::shared_ptr<Infer> create_infer(const std::string& engine_file, int gpuid, float confidence_threshold=0.25f, float nms_threshold=0.5f);
    ...
    #else
        struct stat s;
        bool path_valid_flag = true;
        if(stat(path.c_str(),&s) == 0 )
        {
            if( s.st_mode & S_IFDIR ) //如果file是一个目录路径,判断文件路径不存在
            {
    ...
    static __global__ void decode_kernel(float* predict, int num_bboxes, int num_classes, float confidence_threshold, float* invert_affine_matrix, float* parray, int max_objects){  
        ...
        float* class_confidence = pitem + 4 * num_bboxes;
        float confidence        = *class_confidence;
        int label               = 0;
        for(int i = 1; i < num_classes; ++i){
            class_confidence += num_bboxes;
...
IScrfd::BoxArray scrfd_postprocess(  
    ...
    public:
        ...
        virtual bool preprocess(Job& job, const Mat& image) override{
            ...
            // speed up
            // auto begin_timer = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count() / 1000.0;
            // memcpy(image_host, image.data, size_image);
            // auto end_timer = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count() / 1000.0;
            // std::cout<<"copy used_time(ms): " << end_timer-begin_timer<<std::endl;
            memcpy(affine_matrix_host, job.additional.d2i, sizeof(job.additional.d2i));
            cv::Mat image_cp;
            if(!image.isContinuous())
            {
                cv::resize(image,image_cp,image.size()); //将非连续内存改成连续内存
            }
            else
            {
                image_cp = image;
            }
            checkCudaRuntime(cudaMemcpyAsync(image_device, image_cp.data, size_image, cudaMemcpyHostToDevice, stream_));
            ...
            warp_affine_bilinear_and_normalize_plane(
                image_device,         image_cp.cols * 3,       image_cp.cols,       image_cp.rows, 
        ...
        virtual vector<shared_future<IScrfd::BoxArray>> commits(const vector<Mat>& images) override{
            return ThreadSafedAsyncInferImpl::commits(images);
        }

        virtual std::shared_future<IScrfd::BoxArray> commit(const Mat& image) override{
    ...
    bool compile(
        ...
        if (mode == IScrfd::Mode::INT8) {
            if (!hasEntropyCalibrator) {
                if (!int8_entropy_calibrator_cache_file.empty()) {
                    struct stat s;
                    std::string file_name = int8_entropy_calibrator_cache_file;
                    if(stat(file_name.c_str(),&s) == 0 )
                    {
                        if( s.st_mode & S_IFDIR ) //如果file是一个目录路径,自动添加默认文件名
                        {
                            file_name += "calibration_cache";
                        }
                    }
     
                    INFO("Save calibrator to: %s", file_name.c_str());
                    save_file(file_name, int8Calibrator->getEntropyCalibratorData());
                }
                else {
    ...
    class Scrfd: public IScrfd
    {
        public:
            ...
            std::shared_future<IScrfd::BoxArray> Inference(cv::Mat input_image);
            std::vector<std::shared_future<IScrfd::BoxArray>> Inference(std::vector<cv::Mat>& input_images);
    ...
    int Scrfd::Init(std::string onnx_file, bool build_engine, float confidence_threshold, float nms_threshold)
    {
            ...
            std::string strGitVersion = __git_version__;
            std::string strGitLog = __git_log__;
            INFO("Version: %s", strVersion.c_str());
            INFO("strGitVersion: %s", strGitVersion.c_str());
            INFO("strGitLog: %s", strGitLog.c_str());
        #endif
        std::string engine_file_name = onnx_file.substr(0,onnx_file.find_last_of("."));
        engine_file_name += ".engine.";
        engine_file_name += mode_string(precision_);
        if(build_engine)
        {
            compile(
            precision_, // FP32、FP16、INT8
            batch_size_,            // max batch size
            onnx_file,                  // source 
            engine_file_name,                 // save to
            max_workspace_size_,
            calibration_path_,
    ...
    std::shared_future<IScrfd::BoxArray> Scrfd::Inference(cv::Mat input_image)
    {
        cv::Mat image = input_image;
        return engine_->commit(image);
    }
    std::vector<std::shared_future<Scrfd::BoxArray>> Scrfd::Inference(std::vector<cv::Mat>& input_images)
    {
        return engine_->commits(input_images);
    }
    ...
    {
        std::shared_ptr<IScrfd> scrfd_infer_ptr(new Scrfd());
        return scrfd_infer_ptr;
    }
};