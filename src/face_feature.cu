...
namespace GeelyRobotVisionGpu
{
    ...
    {
    public:
        virtual std::shared_future<cv::cuda::GpuMat> commit(const cv::cuda::GpuMat& image) = 0;
        virtual std::vector<std::shared_future<cv::cuda::GpuMat>> commits(const std::vector<cv::cuda::GpuMat>& images) = 0;
    };
    ...
    */
    ...
    public:
        ...
        virtual bool startup(const string& file, int gpuid, float confidence_threshold, float nms_threshold){
            const float mean[3]={0.5,0.5,0.5}; 
            const float std[3]={0.5,0.5,0.5};
        ...
        virtual std::vector<std::shared_future<cv::cuda::GpuMat>> commits(const std::vector<cv::cuda::GpuMat>& images)
        {
            return ThreadSafedAsyncInferImpl::commits(images);
        }
        virtual std::shared_future<cv::cuda::GpuMat> commit(const cv::cuda::GpuMat& image) override
        {
